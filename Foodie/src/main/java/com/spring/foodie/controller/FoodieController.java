...
public class FoodieController {
	...
	public ModelAndView map_search(HttpServletRequest request, ModelAndView mav) {
		/*
		 * // 글이 여러개 나올 수 있으니 리턴은 list로 주고 초기값 null은 글이 한개도 없을 수 있기 떄문이다. List<SearchVO>
		 * MapSearchList = null; String keyword = request.getParameter("keyword"); //
		 * 폼태그에서 넘어온것 System.out.println("입력한 검색어 : " + keyword); if (keyword == null) {
		 * // 검색어가 없든지 텅빈것 아무것도안누르고 엔터누른경우 keyword = ""; } Map<String, String> paraMap =
		 * new HashMap<>(); paraMap.put("searchWord", keyword); // *** === 검색어가 있을때만
		 * 검색어도 같이 넘겨준다. 없을면 공백이 넘어가버리므로 === *** // if (!"".equals(keyword)) {
		 * mav.addObject("paraMap", paraMap); // 검색을 했을때만 맵을 뷰단으로 보내버린다. } MapSearchList
		 * = service.MapSearchList(paraMap); mav.addObject("MapSearchList",
		 * MapSearchList);
		 */
		String keyword = request.getParameter("keyword"); // 폼태그에서 넘어온것
		System.out.println("입력한 검색어 : " + keyword);
		if (keyword == null) { // 검색어가 없든지 텅빈것 아무것도안누르고 엔터누른경우
			keyword = "";
		}
		...
		/*
		 * String kakao = "https://place.map.kakao.com/main/v/"; String id =
		 * kakao_store_id; String myURL = kakao + id;
		 * 
